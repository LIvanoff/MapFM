...
class MapTRv2(MVXTwoStageDetector):
    ...
    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False, **kwargs):
        ...
        # # Конвертация в numpy
        # bev_seg = np.transpose(torch.sigmoid(outs['segmap'][0]).cpu().numpy(), (1, 2, 0))

        # # Очистка фигур перед созданием новой
        # plt.close('all')  

        # # Создание и визуализация
        # plt.figure(figsize=(10, 5))  
        # plt.imshow(bev_seg.squeeze(-1), cmap='gray')
        # plt.axis('off')  # Отключить оси для чистого изображения
        # plt.show()



        # print("Min:", bev_seg.min(), "Max:", bev_seg.max(), "Shape:", bev_seg.shape)
        # print("Unique values:", np.unique(bev_seg))


        # plt.title('BEV Segmentation')
        # plt.axis('off')
        # plt.show()
        # plt.clf()  # Очистить текущий рисунок
        # plt.cla()  # Очистить текущую ось
        # plt.close()  # Закрыть окно
