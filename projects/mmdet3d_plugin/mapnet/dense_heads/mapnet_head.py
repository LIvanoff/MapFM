...
class MapNetHead(DETRHead):
    ...
    def __init__(self,
                *args,
                ...
                seg_transformer_head=dict(type='SegMaskHead',
                    d_model=256,
                    nhead=8,
        ...
        self.seg_transformer_head = seg_transformer_head
    ...
    def _init_layers(self):
        ...
        reg_branch = []
        if self.query_embed_type == 'instance':
        ...
        if self.aux_seg['segmap']:
            ...
            # self.seg_attention = nn.MultiheadAttention(self.embed_dims, self.embed_dims)
            # self.norm_seg = nn.LayerNorm(self.embed_dims)
            # self.transformer_seg_head = nn.Linear(self.embed_dims, self.aux_seg['segmap_classes'])
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        bev_embed,depth, hs, init_reference, inter_references, seg_mask_embed, seg_query_pos, point_query_pos = outputs
        ...
        if self.aux_seg['segmap']:
            ...
            # import ipdb; ipdb.set_trace()
            hw_lvl = [torch.Size([self.bev_w, self.bev_h])]
            memory = bev_embed.permute(1, 0, 2)
            memory_mask = torch.zeros((1, self.bev_w * self.bev_h), device=bev_embed.device,dtype=torch.bool)
            point_query = hs[-1].permute(1, 0, 2)
            # point_query_pos = point_query_pos.permute(1, 0, 2)
            mask_stuff = self.segmap_head(
                memory,
                memory_mask,
                None,
                point_query,
            ...
            # query2 = self.seg_attention(transformer_seg_feat, transformer_seg_feat, transformer_seg_feat)[0]
            # transformer_seg_feat = transformer_seg_feat + self.dropout(query2)
            # transformer_seg_feat = self.norm_seg(transformer_seg_feat)

            # outputs_segmap = self.transformer_seg_head(transformer_seg_feat) # Convert with the head

            # # outputs_segmap = outputs_segmap.permute(0, 2, 1).view(bs, 1, self.bev_h, self.bev_w)
            # segmap_transposed = outputs_segmap.permute(0, 2, 1) # (BS, num_queries=bev_h*bev_w, embed_dims )
            # segmap_reshaped = segmap_transposed.view(bs, 1, int(math.sqrt(num_queries)), int(math.sqrt(num_queries))) # Convert vector to image. #Maybe need to change number of channels

            # # Apply interpolation to get a segmentation map with desired shape.
            # outputs_segmap = F.interpolate(segmap_reshaped, size=(self.bev_h, self.bev_w), mode='bilinear', align_corners=False)
            
            # # transformer_seg_feat = transformer_seg_feat.permute(0,2,1).view(bs, embed_dims, self.bev_h, self.bev_w)
            # outputs_segmap = self.segmap_head(transformer_seg_feat)
