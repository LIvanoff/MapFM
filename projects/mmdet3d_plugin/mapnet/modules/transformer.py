...
class MapNetPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MapNetPerceptionTransformer, self).__init__(**kwargs)
        if modality == 'fusion':
        ...
        self.z_cfg=z_cfg
        self.query_pos_embedding = self.decoder.query_pos_embedding
    ...
    def init_layers(self):
        ...
        self.seg_query_embedding = nn.Embedding(100, self.embed_dims)
    ...
    def init_weights(self):
        ...
        xavier_init(self.seg_query_embedding, distribution='uniform', bias=0.)
    ...
    def forward(self,
        ...
        seg_query_ids = torch.arange(self.num_seg_queries, device=bev_embed.device) # Get index tensor
        seg_query_embed = self.seg_query_embedding(seg_query_ids) # Use embedding layer
        seg_query_pos = nn.Parameter(torch.randn(self.num_seg_queries, self.embed_dims)).to(bev_embed.device)  # TODO Add learnable pos embedding
        seg_query = seg_query_embed.unsqueeze(1).repeat(1, bs, 1)
        seg_query_pos = seg_query_pos.unsqueeze(1).repeat(1, bs, 1)
        
        ref_h, ref_w = bev_h, bev_w  # Исходное разрешение BEV
        ref_y, ref_x = torch.meshgrid(
            torch.linspace(0.5, ref_h - 0.5, 20, device=bev_embed.device), # Убедитесь, что bev_embed.device правильное!
            torch.linspace(0.5, ref_w - 0.5, 5, device=bev_embed.device)
        )
        ref_y = ref_y.reshape(-1)
        ref_x = ref_x.reshape(-1)
        reference_points_seg = torch.stack((ref_x, ref_y), -1)  # Shape: [ref_h * ref_w, 2]
        reference_points_seg = reference_points_seg.unsqueeze(0).repeat(bs, 1, 1)  # Shape: [bs, ref_h * ref_w, 2]

        reference_points_seg[:, :, 0] /= ref_w
        reference_points_seg[:, :, 1] /= ref_h
        
        img_masks = bev_embed.new_zeros((bs, bev_h, bev_w), dtype=torch.bool) # More efficient way to create the tensor
        mlvl_masks = [F.interpolate(img_masks[None].float(), size=(bev_h, bev_w)).to(torch.bool).squeeze(0)]  # mlvl_feats
        valid_ratios = torch.stack(
            [self.get_valid_ratio(m) for m in mlvl_masks], 1)
        
        # import ipdb;ipdb.set_trace()
        seg_mask_embed, seg_mask_references = self.seg_decoder(  # seg_decoder вычисляется первым
            query=seg_query,
            key=None,
            value=bev_embed,
            query_pos=seg_query_pos,
            reference_points=reference_points_seg,
            spatial_shapes=torch.tensor([[bev_h, bev_w]], device=bev_embed.device), # use bev_embed.device
            level_start_index=torch.tensor([0], device=bev_embed.device), # use bev_embed.device
            mlvl_feats=mlvl_feats,
        ...
        inter_states, inter_references = self.decoder(
            ...
            feat_spatial_shapes=feat_spatial_shapes,
            feat_level_start_index=feat_level_start_index,
            seg_embed=seg_mask_embed,
            **kwargs)

        inter_references_out = inter_references
        point_query_pos = query_pos

        return bev_embed, depth, inter_states, init_reference_out, inter_references_out, seg_mask_embed, seg_query_pos, point_query_pos
